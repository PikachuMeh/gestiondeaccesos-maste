"""
Módulo API para manejo de centros de datos.
"""
...
async def list_centros_datos(
    ...
    if departamento:
        q = q.filter(CentroDatos.departamento.ilike(f"%{departamento}%"))
    try:
        total = q.count()
        if total == 0:
            return CentroDatosListResponse(items=[], total=0, page=page, size=size, pages=0)
        items = q.offset((page - 1) * size).limit(size).all()
        return CentroDatosListResponse(
